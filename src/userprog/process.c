...
/*struct used to share between process_execute() in the nnvoking thread and start_process() inside the newly invoked
   thread. */
struct exec_helper 
  {
...
{
  //struct exec_helper execH;
  //char thread_name[16];

  //char *fn_copy; REMOVING
  tid_t tid;
...
load (const char *file_name, void (**eip) (void), void **esp) //change file name to cmd_line ###
{
  ...
  //char_file_name[NAME_MAX + 2]; //add a file name variable here, the file_name and cmd_line are different! ###
  ...
  file = filesys_open (file_name); //set the thread's bin file to this as well! it is superhelpful to have each thread have a pointer to the file they are using for when you need to close it in process_exit ###
  if (file == NULL) 
    {
  ...
  *eip = (void (*) (void)) ehdr.e_entry;
  ...
  file_close (file); //remove thise since thread has its own file ###
  return success;
}

/* load() helpers. */
  ...
  if (kpage != NULL) 
    {
      ...
      if (success)
        *esp = PHYS_BASE - 12; ////////// TEMP WORKAROUND: Works for any test program that doesn't examine arguments
      else
        palloc_free_page (kpage);
    }
  return success;
